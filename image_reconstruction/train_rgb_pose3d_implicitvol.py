#!/usr/bin/env python3
"""
基于ImplicitVol架构的RGB Pose3D训练脚本
使用位置编码(PE)和带残差连接的MLP网络预测像素值
参照train_rgb_pose3d_hash_multi_expert.py的打印格式
"""
...
from models.INR_ImplicitVol import INR_ImplicitVol
...
def compute_loss(output_pred, gt_img_flat, recon_weight=100.0):
    """
    计算重建损失
    ...
    """
    # 获取预测结果
    pred = output_pred.get('nonmanifold_pnts_pred', None)  # (B, out_dim, N)
    if pred is None:
        raise ValueError("无法找到nonmanifold_pnts_pred")
    
    # 调整形状: (B, out_dim, N) -> (B, N, out_dim)
    if pred.dim() == 3:
        pred = pred.permute(0, 2, 1)  # (B, N, out_dim)
    
    # 对于灰度图，确保形状匹配
    if pred.shape[-1] == 1:
        pred = pred.squeeze(-1)  # (B, N)
        gt = gt_img_flat.squeeze(-1)  # (B, N)
    else:
        gt = gt_img_flat  # (B, N, C)
    
    # 重建损失 (MSE)
    recon_loss = torch.mean((pred - gt) ** 2) * recon_weight
...
def evaluate_test_set(model, dataloader, device, cfg):
    ...
    with torch.no_grad():
        for data in dataloader:
            ...
            # 获取预测结果
            pred = output_pred.get('nonmanifold_pnts_pred', None)
            if pred is None:
            ...
            if pred.shape[-1] == 1:
                pred = pred.squeeze(-1)  # (B, N)
                gt_flat = gt_img_flat.squeeze(-1)  # (B, N)
            else:
                gt_flat = gt_img_flat  # (B, N, C)
                if pred.dim() == 2:
                    pred = pred.unsqueeze(0)  # (N,) -> (1, N)
            
            # 限制预测值到[0,1]范围
            pred_clamped = torch.clamp(pred, 0.0, 1.0)
            
            # 计算指标
            mse = torch.mean((pred_clamped - gt_flat) ** 2)
            psnr = 10 * torch.log10(1.0 / mse) if mse > 0 else torch.tensor(float('inf'))
            mae = torch.mean(torch.abs(pred_clamped - gt_flat))
            
            # 计算SSIM
            pred_2d = pred_clamped.reshape(B, H, W, 1).detach().cpu().numpy()
            ...
            if lpips_model is not None:
                # 将预测图像重塑为 (B, C, H, W) 格式
                if pred_clamped.dim() == 1:
                    pred_img_2d = pred_clamped.reshape(1, H, W)
                elif pred_clamped.dim() == 2:
                    pred_img_2d = pred_clamped.reshape(B, H, W)
                else:
                    pred_img_2d = pred_clamped.reshape(B, H, W)
...
def save_prediction_samples(model, dataloader, device, output_dir, epoch, num_samples=3, prefix='train'):
    """保存预测样本"""
    model.eval()
    ...
    with torch.no_grad():
        for i, data in enumerate(dataloader):
            ...
            B, C, H, W = gt_img.shape
            if pred.dim() == 2:  # (B, H*W)
                pred = pred.reshape(B, H, W).unsqueeze(1)
            elif pred.dim() == 1:  # (H*W,)
                pred = pred.reshape(H, W).unsqueeze(0).unsqueeze(0)
            ...
            img_pil.save(os.path.join(output_dir, f'epoch_{epoch}_{prefix}_sample_{i}_implicitvol.png'))
...
def main():
    ...
    for epoch in range(total_epochs):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            # 计算损失
            loss_dict = compute_loss(output_pred, gt_img_flat, recon_weight=recon_weight)
            ...
            if batch_idx % 10 == 0:
                ...
                gt_flat = gt_img_flat.squeeze(-1) if C == 1 else gt_img_flat
                
                mse = torch.mean((pred - gt_flat) ** 2)
                psnr = 10 * torch.log10(1.0 / mse) if mse > 0 else torch.tensor(float('inf'))
                mae = torch.mean(torch.abs(pred - gt_flat))
                
                # 计算SSIM
                pred_2d = pred.reshape(B, H, W, 1).detach().cpu().numpy()
                gt_2d = gt_img_flat.reshape(B, H, W, C).detach().cpu().numpy()
        ...
        if (epoch + 1) % test_interval == 0 or epoch == total_epochs - 1:
            ...
            test_metrics = evaluate_test_set(model, test_loader, device, cfg)
            print(f"测试集指标:")
        ...
        save_prediction_samples(model, test_loader, device, vis_outdir, epoch, num_samples=10, prefix='test')
        print(f"  测试集可视化保存到: {vis_outdir}")

        # 保存模型
        if epoch % save_interval == 0 or epoch == total_epochs - 1:
            model_path = os.path.join(model_outdir, f'{cfg["MODEL"]["model_name"]}_implicitvol_{epoch}.pth')
            ...
            if epoch % vis_interval == 0 or epoch == total_epochs - 1:
                print(f"  生成训练可视化...")
                save_prediction_samples(model, train_loader, device, vis_outdir, epoch, num_samples=3, prefix='train')
...
def parse_args():
    import argparse
    parser = argparse.ArgumentParser(description='Training RGBPose3D with ImplicitVol')
    parser.add_argument('--config', type=str, default='configs/config_RGB_pose3d_implicitvol_uvfdata2.yaml',
                        help='Path to config file')
    parser.add_argument('--logdir', type=str, default='./log/implicitvol',
