#!/usr/bin/env python3
"""
基于原始NeRF架构的RGB Pose3D训练脚本
使用位置编码(PE)和MLP网络预测像素值
"""
...
from models.INR_NeRF import INR_NeRF
...
def save_prediction_samples(model, dataloader, device, output_dir, epoch, num_samples=3, prefix='train'):
    ...
    with torch.no_grad():
        for i, data in enumerate(dataloader):
            ...
            img_pil.save(os.path.join(output_dir, f'epoch_{epoch}_{prefix}_sample_{i}_nerf.png'))
...
def main():
    ...
    for epoch in range(total_epochs):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            # 计算指标
            if batch_idx % 10 == 0:
                pred = loss_dict['pred']  # 已经是clamped的
                if pred.dim() == 1:
                    pred = pred.unsqueeze(0)
                gt_flat = gt_img_flat.squeeze(-1) if C == 1 else gt_img_flat
                
                # 确保pred在[0,1]范围内（与测试集评估保持一致）
                pred_clamped = torch.clamp(pred, 0.0, 1.0)
                mse = torch.mean((pred_clamped - gt_flat) ** 2)
                psnr = 10 * torch.log10(1.0 / mse) if mse > 0 else torch.tensor(float('inf'))
                mae = torch.mean(torch.abs(pred_clamped - gt_flat))
        ...
        if epoch % save_interval == 0 or epoch == total_epochs - 1:
            model_path = os.path.join(model_outdir, f'{cfg["MODEL"]["model_name"]}_nerf_{epoch}.pth')
...
def parse_args():
    import argparse
    parser = argparse.ArgumentParser(description='Training RGBPose3D with NeRF')
    parser.add_argument('--config', type=str, default='configs/comparison_methods/nerf_uvfdata2.yaml',
                        help='Path to config file')
    parser.add_argument('--logdir', type=str, default='./log/nerf',
