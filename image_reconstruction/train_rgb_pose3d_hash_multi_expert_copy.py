...
def evaluate_test_set(model, dataloader, device, cfg):
    ...
    with torch.no_grad():
        for data in dataloader:
            ...
            # 使用top1专家的预测结果（而不是加权平均）
            top1_pred = output_pred.get('selected_nonmanifold_pnts_pred', None)
            if top1_pred is None:
                # 如果没有selected结果，回退到加权平均
                ...
                top1_pred = torch.sum(q * all_expert_preds, dim=1)  # (B, H*W)
            
            # 确保形状正确
            if top1_pred.dim() == 3:  # (1, n_nm, out_dim)
                top1_pred = top1_pred.squeeze(0)  # (n_nm, out_dim)
            elif top1_pred.dim() == 2:  # (1, n_nm) 或 (n_nm, out_dim)
                if top1_pred.shape[0] == 1:
                    top1_pred = top1_pred.squeeze(0)  # (n_nm,)
            elif top1_pred.dim() == 1:  # (n_nm,)
                pass
            
            # 对于灰度图，确保是1维的 (n_nm,)
            # 对于RGB图，应该是 (n_nm, 3)，但当前模型只输出单通道，所以这里假设是灰度图
            if top1_pred.dim() == 2 and top1_pred.shape[-1] == 1:
                top1_pred = top1_pred.squeeze(-1)  # (n_nm,)
            
            # 限制预测值到[0,1]范围 (用于指标计算和可视化)
            top1_pred_clamped = torch.clamp(top1_pred, 0.0, 1.0)
            ...
            mse = torch.mean((top1_pred_clamped - gt_flat) ** 2)
            psnr = 10 * torch.log10(1.0 / mse) if mse > 0 else torch.tensor(float('inf'))
            mae = torch.mean(torch.abs(top1_pred_clamped - gt_flat))
            
            # 计算SSIM (使用clamp后的值)
            pred_2d = top1_pred_clamped.reshape(B, H, W, 1).detach().cpu().numpy()
            ...
            if lpips_model is not None:
                # 将预测图像重塑为 (B, C, H, W) 格式
                # top1_pred_clamped可能是(B, H*W)或(H*W,)形状，需要重塑为(B, H, W)
                if top1_pred_clamped.dim() == 1:
                    # 如果是1维，假设是(H*W,)，重塑为(1, H, W)
                    pred_img_2d = top1_pred_clamped.reshape(1, H, W)
                elif top1_pred_clamped.dim() == 2:
                    # 如果是2维，假设是(B, H*W)，重塑为(B, H, W)
                    pred_img_2d = top1_pred_clamped.reshape(B, H, W)
                else:
                    # 其他情况，尝试重塑为(B, H, W)
                    pred_img_2d = top1_pred_clamped.reshape(B, H, W)
...
def main():
    ...
    for epoch in range(total_epochs):
        ...
        if (epoch + 1) % test_interval == 0 or epoch == total_epochs - 1:
            ...
            # 保存测试集可视化
            print(f"  生成测试集可视化...")
            save_prediction_samples_with_expert_info(SINR, test_loader, device, vis_outdir, epoch, num_samples=3, prefix='test')
