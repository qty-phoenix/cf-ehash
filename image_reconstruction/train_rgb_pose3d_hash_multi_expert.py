...
from models.INR_MoE_HashHybrid import INR_MoE_HashHybrid
...
def compute_gradient_aware_loss(output_pred, gt_img_flat, q, balance_weight=10.0, recon_weight=100.0, use_balance_loss=True):
    """
    ...
    Args:
        ...
        balance_weight: 平衡损失权重（仅在use_balance_loss=True时使用）
        recon_weight: 重建损失权重
        use_balance_loss: 是否使用平衡损失（如果使用Auxiliary-Loss-Free方法，设为False）
    """
    # 获取所有专家的预测结果
    all_expert_preds = output_pred.get('nonmanifold_pnts_pred', None)  # (1, n_experts, n_nm, out_dim)
    if all_expert_preds is None:
        raise ValueError("无法找到nonmanifold_pnts_pred")
    
    # 确保形状正确
    if all_expert_preds.dim() == 4:  # (1, n_experts, n_nm, out_dim)
        all_expert_preds = all_expert_preds.squeeze(-1) if all_expert_preds.shape[-1] == 1 else all_expert_preds  # (1, n_experts, n_nm)
    elif all_expert_preds.dim() == 3:  # (1, n_experts, n_nm)
        pass
    else:
    ...
    # 2. Manager平衡损失（可选，如果使用Auxiliary-Loss-Free方法则跳过）
    ...
    if use_balance_loss:
        ...
        # Auxiliary-Loss-Free: 不使用平衡损失，只使用重建损失
        balance_loss = torch.tensor(0.0, device=recon_loss.device)
...
def evaluate_test_set(model, dataloader, device, cfg):
    ...
    with torch.no_grad():
        for data in dataloader:
            ...
            # 对于灰度图，确保是1维的 (n_nm,)
            # 对于RGB图，应该是 (n_nm, 3)，但当前模型只输出单通道，所以这里假设是灰度图
            ...
            if lpips_model is not None:
                # 将预测图像重塑为 (B, C, H, W) 格式
                # weighted_pred_clamped可能是(B, H*W)或(H*W,)形状，需要重塑为(B, H, W)
                if weighted_pred_clamped.dim() == 1:
                    # 如果是1维，假设是(H*W,)，重塑为(1, H, W)
                    pred_img_2d = weighted_pred_clamped.reshape(1, H, W)
                elif weighted_pred_clamped.dim() == 2:
                    # 如果是2维，假设是(B, H*W)，重塑为(B, H, W)
                    pred_img_2d = weighted_pred_clamped.reshape(B, H, W)
                else:
                    # 其他情况，尝试重塑为(B, H, W)
                    pred_img_2d = weighted_pred_clamped.reshape(B, H, W)
                
                # 确保是(B, H, W)格式，然后添加通道维度: (B, H, W) -> (B, 1, H, W)
                if pred_img_2d.dim() == 2:
                    pred_img_2d = pred_img_2d.unsqueeze(0)  # (H, W) -> (1, H, W)
                ...
                # pred_img来自weighted_pred_clamped，应该已经在[0,1]范围内，但为了安全再次clamp
                pred_img = torch.clamp(pred_img, 0.0, 1.0)
                # gt_img应该已经在[0,1]范围内（数据集使用ToTensor()），但为了安全再次clamp
...
def save_prediction_samples_with_expert_info(model, dataloader, device, output_dir, epoch, num_samples=3, prefix='train'):
    ...
    with torch.no_grad():
        for i, data in enumerate(dataloader):
            ...
            if all_expert_preds.dim() == 4:
                all_expert_preds = all_expert_preds.squeeze(-1) if all_expert_preds.shape[-1] == 1 else all_expert_preds
            ...
            img_pil.save(os.path.join(output_dir, f'epoch_{epoch}_{prefix}_sample_{i}_hash_multi_expert.png'))
            ...
            if q is not None:
                ...
                expert_img.save(os.path.join(output_dir, f'epoch_{epoch}_{prefix}_expert_selection_{i}_hash_multi_expert.png'))
                ...
                if n_experts == 4:
                    ...
                    expert_img_color.save(os.path.join(output_dir, f'epoch_{epoch}_{prefix}_expert_selection_color_{i}_hash_multi_expert.png'))
...
def main():
    ...
    # 创建测试集指标记录文件
    metrics_file = os.path.join(logdir, 'test_metrics.json')
    test_metrics_history = []  # 存储所有epoch的测试指标
    ...
    # 检查是否使用Auxiliary-Loss-Free Load Balancing
    use_expert_bias = cfg['MODEL'].get('use_expert_bias_for_load_balancing', False)
    use_balance_loss = not use_expert_bias  # 如果使用expert_bias，则不使用balance_loss
    
    print(f"损失类型: {loss_type}")
    print(f"重建损失权重: {recon_weight}")
    if use_expert_bias:
        print(f"✅ 使用 Auxiliary-Loss-Free Load Balancing (专家偏置策略)")
        print(f"   平衡损失: 已禁用")
        bias_update_rate = cfg['MODEL'].get('expert_bias_update_rate', 0.01)
        print(f"   偏置更新率: {bias_update_rate}")
    else:
        print(f"平衡损失权重: {balance_weight} (传统方法)")

    # 创建模型
    SINR = INR_MoE_HashHybrid(cfg)
    ...
    # 检查多专家哈希编码参数
    hash_encoder_params = []
    if hasattr(SINR.decoder_input_encoding_module, 'multi_expert_hash_encoder') and \
       SINR.decoder_input_encoding_module.multi_expert_hash_encoder is not None:
        hash_encoder_params = list(SINR.decoder_input_encoding_module.multi_expert_hash_encoder.parameters())
        print(f"检测到多专家哈希编码，参数数量: {sum(p.numel() for p in hash_encoder_params):,}")
    ...
    for epoch in range(total_epochs):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            expert_usage_batch = loss_dict['expert_usage'].detach().cpu().numpy()
            epoch_expert_usage.append(expert_usage_batch)
            
            # 添加：如果使用expert_bias方法，在每个batch后更新偏置（可选：也可以每个epoch更新一次）
            if use_expert_bias and hasattr(SINR.manager_net, 'update_expert_biases'):
                # 获取当前batch的专家使用情况
                expert_usage_tensor = loss_dict['expert_usage']  # (n_experts,)
                SINR.manager_net.update_expert_biases(expert_usage_tensor.unsqueeze(0))  # 添加batch维度
        ...
        avg_balance_loss = np.mean(epoch_balance_losses) if use_balance_loss else 0.0
        ...
        # 添加：如果使用expert_bias方法，在每个epoch结束时基于平均使用情况再次更新偏置
        # （这可以作为batch更新的补充，提供更稳定的更新）
        if use_expert_bias and hasattr(SINR.manager_net, 'update_expert_biases'):
            expert_usage_tensor = torch.from_numpy(expert_usage_avg).to(device)
            SINR.manager_net.update_expert_biases(expert_usage_tensor.unsqueeze(0))
            
            # 打印当前偏置值（用于监控）
            if epoch % 10 == 0 or epoch == total_epochs - 1:
                biases_str = ", ".join([f"E{i}: {bias:.4f}" for i, bias in enumerate(SINR.manager_net.expert_biases.cpu().numpy())])
                print(f"  专家偏置: [{biases_str}]")
        ...
        if (epoch + 1) % test_interval == 0 or epoch == total_epochs - 1:
            ...
            metrics_entry = {
                ...
                'lpips': float(test_metrics['lpips']) if test_metrics['lpips'] is not None else None
            }
            test_metrics_history.append(metrics_entry)
            
            # 保存到JSON文件
            with open(metrics_file, 'w') as f:
                json.dump(test_metrics_history, f, indent=2)
            print(f"📊 测试集指标已保存到: {metrics_file}")
        
        # 🔥 每个epoch都保存测试集可视化（前10个）
        print(f"  生成测试集可视化 (Epoch {epoch+1})...")
        save_prediction_samples_with_expert_info(SINR, test_loader, device, vis_outdir, epoch, num_samples=10, prefix='test')
        print(f"  测试集可视化保存到: {vis_outdir}")

        # 保存模型
        if epoch % save_interval == 0 or epoch == total_epochs - 1:
            model_path = os.path.join(model_outdir, f'{cfg["MODEL"]["model_name"]}_hash_multi_expert_{epoch}.pth')
...
def parse_args():
    import argparse
    parser = argparse.ArgumentParser(description='Training RGBPose3D with Multi-Expert Hash Encoding')
    parser.add_argument('--config', type=str, default='configs/config_RGB_pose3d_hash_multi_expert.yaml',
                        help='Path to config file')
    parser.add_argument('--logdir', type=str, default='./log/hash_multi_expert',
