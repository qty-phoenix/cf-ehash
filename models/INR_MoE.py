...
class Manager(nn.Module):
    def __init__(self, cfg, in_dim, module_name=''):
        super().__init__()
        self.n_experts = cfg['n_experts']# 专家数量
        self.point_dim = cfg['in_dim']# 输入维度
        self.temperature = cfg['manager_softmax_temperature']# Softmax温度参数
        # 激活函数字典
...
class INR_MoE(nn.Module):
    def __init__(self, cfg_all):
        ...
        self.decoder = ParallelFullyConnectedNN(self.n_experts, decoder_first_layer_dim, cfg['out_dim'],
                                        ...
                                        module_name='decoder', cfg=cfg)  # SIREN decoder
    ...
    def forward(self, non_mnfld_pnts, mnfld_pnts=None, **kwargs):
        ...
        if not self.share_encoder:
            ...
            # 如果共享编码器，使用专家编码的特征作为管理器输入
            encoded_non_mnfld_pnts_manager = encoded_non_mnfld_pnts_experts  # (1,n_nm,d)
