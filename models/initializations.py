...
def waveWeightInit(decoder):
    """
    WaveLayer的权重初始化方法
    基于SIREN初始化但适配WaveLayer的特殊结构
    """
    for i in range(len(decoder.weights_list)):
        fan_in = decoder.weights_list[i].shape[-2]
        fan_out = decoder.weights_list[i].shape[-1]
...
def multiplicativeWeightInit(decoder):
    """
    MultiplicativeActivation的权重初始化方法
    由于MultiplicativeActivation内部有自己的参数，这里主要初始化线性层
    """
    for i in range(len(decoder.weights_list)):
        fan_in = decoder.weights_list[i].shape[-2]
        fan_out = decoder.weights_list[i].shape[-1]
        
        # 使用SIREN风格的初始化，但稍微保守一些
        if i == 0:
            bound = 1 / fan_in
        else:
            bound = np.sqrt(6 / fan_in) / 30 * 0.5  # 比标准SIREN小一半
        
        nn.init.uniform_(decoder.weights_list[i], -bound, bound)
        nn.init.uniform_(decoder.biases_list[i], -bound * 0.1, bound * 0.1)  # 偏置更小
