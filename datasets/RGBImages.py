...
class RGBINRDataset(torch.utils.data.Dataset):
    def __init__(self, data_cfg, img_id):
        ...
        if self.downsample_factor > 1:
            # 支持小数下采样
            new_width = int(self.sidelength[0] / self.downsample_factor)
            new_height = int(self.sidelength[1] / self.downsample_factor)
            self.sidelength = (new_width, new_height)
    ...
    def apply_gaussian_filter(self, image, threshold_freq, high_pass=False):
        ...
        # Merge the filtered color channels back into a color images
    ...
    def get_freq_domain(self):
        '''Computes the frequency domain of the images.'''
    ...
    def get_sam_segments(self):
        ...
        # Assuming `segments` is your 1D array of labels and `img` is your images
    ...
    def get_grid_segmentation(self):
        '''Creates a grid segmentation of the images. The number of segments is determined by the n_segments parameter.'''
    ...
    def circular_segmentation(self):
        ''' Create a circular segmentation around the images center.'''
    ...
    def __getitem__(self, idx):
        ...
        img = img.permute(1, 2, 0).reshape(-1, self.img_channels) # flatten the images to match coordinates
