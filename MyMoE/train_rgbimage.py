...
def main(args):
    ...
    # 在2D图像的重建中，样本只有本身的一张图像，所以用epoch代表样本数从而兼容原模型
    # the training stage handler uses n_sampled (because of the 3D dataset), this hack is to allow support for RGB with minimal changes
    ...
    # cfg里stage内容是指前80全部训练，后20只训练专家网络
    criterion = training_stage_handler.criterion
    # criterion 实际上是 RGBImageLoss 类的实例，它包含主损失项和正则化项和权重分配
